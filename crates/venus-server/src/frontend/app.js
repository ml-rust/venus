...
function send(msg) {
    if (state.connected && state.ws) {
        ...
        console.warn('[Venus] Cannot send, not connected:', msg.type);
    }
}
...
function createCodeCellElement(cell) {
    ...
    const div = document.createElement('div');
    const dirtyClass = !isDefinition && cell.dirty ? ' dirty' : '';
    div.className = `cell ${isDefinition ? 'cell-definition' : cell.status}${dirtyClass}`;
    div.id = `cell-${cell.id}`;
    div.dataset.cellId = cell.id;
...
function createEditor(cellId, source) {
    ...
    editor.onDidChangeModelContent(() => {
        updateHeight();
        // Mark cell as dirty and update content locally (don't send to server yet)
        const cell = state.cells.get(cellId);
        if (cell) {
            cell.dirty = true;
...
function updateCellStatus(cellId) {
    ...
    // Update cell class (include dirty state)
    const dirtyClass = cell.dirty ? ' dirty' : '';
    cellEl.className = `cell ${cell.status}${dirtyClass}`;
...
function updateCellOutput(cellId) {
    ...
    if (output.html) {
        contentType = 'html';
        // HTML is generated by trusted cell code via Render trait
        // In production with untrusted code, would require sanitization
        content = output.html;
...
function executeCell(cellId) {
    // Get the editor content and send it to server before executing
    const editor = state.editors.get(cellId);
    if (editor) {
        const source = editor.getValue();
        send({ type: 'cell_edit', cell_id: cellId, source: source });
    }
...
// Run All button onclick is set dynamically in updateExecutionUI()
// to switch between executeAll and interruptExecution
...
document.addEventListener('change', (e) => {
    ...
    switch (widgetType) {
        ...
        case 'select':
            ...
            value = isNaN(selectedIdx) ? 0 : selectedIdx;
            break;
        case 'checkbox':
...
// Initialize Run All button (sets onclick handler)
updateExecutionUI();
